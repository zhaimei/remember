...
int find((int)zggz*a) //查询职工工资数据
{
	...
	{
		printf("员工工号为%s",a[i].id);
		printf("员工姓名为%s",a[i].name);
		printf("岗位工资为%f",a[i].gslary);
		printf("薪级工资为%f",a[i].xslary);
		printf("职务津贴为%f",a[i].zslary);
		printf("绩效工资为%f",a[i].jslary);
		printf("应发工资为%f",a[i].sumslary);
		printf("个人所得税为%f",a[i].personaltax);
		printf("实发工资为%f",a[i].realslary);
		x++;
	}
	if(x==0)
	{
